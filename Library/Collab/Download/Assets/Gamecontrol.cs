...
public class Gamecontrol : MonoBehaviour {
	...
	private RawImage[] itemPlayer1 = new RawImage[11];
	private RawImage[] itemPlayer2 = new RawImage[11];
	...
	void Start() {
		...
		if (Need.scene_index == 1) {
			...
			interactiveField = GameObject.Find ("InteractiveField").GetComponent<Text> ();
			for (int j = 0; j <= 10; j++) {
				itemPlayer1[j] = GameObject.Find ("IconsPlayer_1").transform.GetChild (j).GetComponent<RawImage> ();
				itemPlayer1 [j].enabled = false;
				itemPlayer2[j] = GameObject.Find ("IconsPlayer_2").transform.GetChild (j).GetComponent<RawImage> ();
				itemPlayer2 [j].enabled = false;
			}
		}
	...
	private void showIcon() {
		if (ItemHandler.eventFlags [(int)ItemHandler.Events.BallSpeedUp]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [0].enabled = true;
				itemPlayer1 [6].enabled = true;
			} else {
				itemPlayer2 [0].enabled = true;
				itemPlayer2 [6].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.BallRespawn]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [0].enabled = true;
				itemPlayer1 [10].enabled = true;
			} else {
				itemPlayer2 [0].enabled = true;
				itemPlayer2 [10].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.BallDisappear]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [0].enabled = true;
				itemPlayer1 [8].enabled = true;
			} else {
				itemPlayer2 [0].enabled = true;
				itemPlayer2 [8].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.BallDirectionInversion]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [0].enabled = true;
				itemPlayer1 [4].enabled = true;
			} else {
				itemPlayer2 [0].enabled = true;
				itemPlayer2 [4].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.CameraShaking]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [2].enabled = true;
				itemPlayer1 [7].enabled = true;
			} else {
				itemPlayer2 [2].enabled = true;
				itemPlayer2 [7].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.NothingHappend]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [3].enabled = true;
			} else {
				itemPlayer2 [3].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.PanelInversion]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [1].enabled = true;
				itemPlayer1 [4].enabled = true;
			} else {
				itemPlayer2 [1].enabled = true;
				itemPlayer2 [4].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.PanelShrinking]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [1].enabled = true;
				itemPlayer1 [5].enabled = true;
			} else {
				itemPlayer2 [1].enabled = true;
				itemPlayer2 [5].enabled = true;
			}
		}
		if(ItemHandler.eventFlags[(int)ItemHandler.Events.PanelSlowdown]) {
			if (Need.lastItem == Need.player1_name) {
				itemPlayer1 [1].enabled = true;
				itemPlayer1 [9].enabled = true;
			} else {
				itemPlayer2 [1].enabled = true;
				itemPlayer2 [9].enabled = true;
			}
		}
	}
	...
	private void hideIcon() {
		if (Need.lastItem == Need.player1_name) {
			for (int i = 0; i <= 10; i++) {
				if (itemPlayer1 [i].enabled) {
					itemPlayer1 [i].enabled = false;
				}
			}
		} else {
			for (int i = 0; i <= 10; i++) {
				if (itemPlayer2 [i].enabled) {
					itemPlayer2 [i].enabled = false;
				}
			}
		}
	}
